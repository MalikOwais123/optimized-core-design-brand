...
const StyledBtn = ({ children, selected, type, btnColor, ...restProps }) => (
  <>
    <Button
      ...
      style={
        (!(selected === type) ? { border: "none", transiton: "all 300ms ease-in" } : { border: "none", transiton: "all 300ms ease-in" },
        { borderRadius: "6px", transform: "scale(1.1)", padding: "8px", transiton: "all 300ms ease-in" })
      }
      {...restProps}>
      {children}
    </Button>
...
const ShowCase = ({  backgroundColor = "#008080", btnColor = "white", data}) => {
  ...
  const filterData = data || showCaseData.filter((item) => {
    ...
    <>
      <Section background={backgroundColor}>
        <div className={classList.main}>
          {!data  && (
            ...
            <div className={classList.headingWidth}>
              ...
              <Paragraph fontWeight="semi-bold" color={btnColor}>
                ...
                Development, Custom Website Design Services, Best Shopify Design
                Websites, Web Design and Digital Marketing Agency, we are a one
                in all solution design company for you.
              </Paragraph>
              <Header color={btnColor} fontWeight={"bold"}>
                PORTFOLIO
              </Header>
          ...
          <Header color="white" fontWeight="bold" style={{marginBottom: "20px"}} >PORTFOLIO</Header>
          )
          }
            ...
            {selected && (
              <>
                ...
                {filterData.slice(0, data ? 9 : 6).map((item, index) => (
                  ...
                  <AnimatedCards index={index} key={item.img} setShowModal={setShowModal} setCurrentImage={setCurrentImage} item={item} />
